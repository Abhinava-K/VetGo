...
export default function RequestsScreen() {
  ...
  const renderRequestItem = ({ item }: { item: any }) => {
    ...
    const doctorName = item.acceptedBy?.name
      ...
      : item.mockDoctor?.name || 'Assigned Doctor';
    ...
    <View style={[
                  ...
                  <View style={styles.reporterRow}>
                    ...
                    <View style={{ flex: 1, marginLeft: 10 }}>
                      <Text style={[styles.reporterNameText, { color: theme.text }]}>
                        {selectedCase.acceptedBy?.name
                          ? `Dr. ${selectedCase.acceptedBy.name.first} ${selectedCase.acceptedBy.name.last}`.trim()
                          : selectedCase.mockDoctor?.name || 'Assigned Doctor'}
                      </Text>
                      {selectedCase.doctorPhone ? (
                        <Text style={{ color: theme.textSecondary, fontSize: 13 }}>📞 {selectedCase.doctorPhone}</Text>
                      ) : (
      ...
      {selectedCase && (
        <ReportModal
          ...
          reportedUserId={selectedCase.acceptedBy?._id || selectedCase.acceptedBy}
          reporterRole="USER"
          isPostService={true}
        />
      )}
